#ifndef CAR_COM_OBJECTS_RACECAR_H
#define CAR_COM_OBJECTS_RACECAR_H


#include <cstdint>
#include <cstring>
#include <car/com/objects/time.h>
#include <car/com/objects/pose.h>
#include <car/com/objects/twist.h>
#include <car/com/objects/wheel.h>
#include <car/com/objects/description_vehicle.h>

#if defined(__amd64__)
#include <iostream>
#include <sstream>
#endif

namespace car {
namespace com {
namespace objects {
    
    
class  RaceCar {
public:
    RaceCar(){};
    Time stamp;
    Wheel wheels[4];
    WheelTimeStamp wheels_tstamp;
    TwistStamped twist;
    PoseStamped  pose;
    
    
#if defined(__amd64__)
    friend std::ostream &operator << ( std::ostream &os, const RaceCar &o ) {
        os << o.getToString();
        return os;
    };
    std::string getToString() const {
        std::stringstream ss;
        ss << "[ " << stamp;
        for(size_t i = 0; i < 4; i++) ss << ", " << wheels;
        ss << ", " << pose << "]" << std::endl;
        return ss.str();
    }
    std::string getToStringReadable() const {
        std::stringstream ss;
        ss << stamp.getToStringReadable().c_str() << std::endl;
        ss << "Wheels       :" << wheels_tstamp.getToStringReadable() << std::endl;
        for(size_t i = 0; i < 4; i++) ss << " " << i << ": " <<  wheels[i].getToStringReadableSimple() << std::endl;
        ss << "Twist        : "  << twist.getToStringReadable() << std::endl;
        ss << "Pose         : "  << pose.getToStringReadable() << std::endl;
        return ss.str();
    }
#endif
};

class  CarConfig {
public:
    CarConfig(){};
    Pose wheel_mount_points[4];
    static const CarConfig getAckermannConfig(float wheel_displacement, float axis_displacement){
        CarConfig config;
        config.wheel_mount_points[FRONT_WHEEL_LEFT ] = Pose(axis_displacement,  wheel_displacement/2.0, 0.);
        config.wheel_mount_points[FRONT_WHEEL_RIGHT] = Pose(axis_displacement, -wheel_displacement/2.0, 0.);
        config.wheel_mount_points[REAR_WHEEL_LEFT ] = Pose(0,  wheel_displacement/2.0, 0.);
        config.wheel_mount_points[REAR_WHEEL_RIGHT] = Pose(0, -wheel_displacement/2.0, 0.);
        return config;
    }
#if defined(__amd64__)
    friend std::ostream &operator << ( std::ostream &os, const CarConfig &o ) {
        os << o.getToString();
        return os;
    };
    std::string getToString() const {
        std::stringstream ss;
        ss << "[ ";
        for(size_t i = 0; i < 4; i++) ss << ", " <<  wheel_mount_points[i];
        ss << "]" << std::endl;
        return ss.str();
    }
    std::string getToStringReadable() const {
        std::stringstream ss;
        ss << "Wheels mount points:" << std::endl;
        for(size_t i = 0; i < 4; i++) ss << " " << i << ": " <<  wheel_mount_points[i].getToStringReadable() << std::endl;
        return ss.str();
    }
#endif

};
};
};
};


#endif // CAR_COM_OBJECTS_CAR_H
