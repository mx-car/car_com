#ifndef MX_SERIAL_BASE_H
#define MX_SERIAL_BASE_H

#include <stdint.h>
#include <stdlib.h>
#include <cstring>
#include <mx/serial/objects.h>
...
struct TMessageHeader {
    TMessageHeader();
    TMessageHeader ( uint16_t size, uint32_t seq, const mx::serial::TTime &stamp );
    uint16_t size;                       /// total message size if size
    uint32_t seq;                        /// consecutively increasing ID
    mx::serial::TTime stamp;                          /// time since epoch
    void zeros();
};
